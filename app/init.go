...
var HeaderFilter = func(c *revel.Controller, fc []revel.Filter) {
  ...
  c.Response.Out.Header().Set("X-Powered-By", "Voyager Innovations")

  //-- Prevent browser from loading page within an iframe
  c.Response.Out.Header().Set("X-Frame-Options", "DENY")

  //-- Enables the Cross-site scripting (XSS) filterbuilt into most recent web browsers
  c.Response.Out.Header().Set("X-XSS-Protection", "1; mode=block")
  ...
  c.Response.Out.Header().Set("Cache-Control", "no-cache, no-store, must-revalidate, private")
